      ...
      const fetchRentals = async () => {
        try {
          ...
          if (rentalData.length > 0) {
            ...
            const coordsPromises = rentalData.map((property) => geocodeAddress(property.address));
            const coords = await Promise.all(coordsPromises);
            setCoordinates(coords.filter(coord => coord !== null));
            setNoProperties(false); // Rentals found, so no "no properties" message
          } else {
            // No rentals found, fetch the city's coordinates to center the map
            await fetchCityCoordinates(city);
            setNoProperties(true); // No properties found
          }
        } catch (error) {
          ...
          {coordinates.length > 0 ? (
            coordinates.map((coord, index) => (
              <Marker
                ...
                icon={customIcon}
              >
                <Popup>{rentals[index]?.name || 'No name available'}</Popup>
              </Marker>
            ))
          ) : (
            noProperties && (
